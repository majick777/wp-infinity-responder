...
# Load the regexps
$query = "SELECT DISTINCT * FROM ".$infrespbounceregs;
...
$query = "SELECT * FROM ".$infrespbouncers." WHERE Enabled = '1' ORDER BY BouncerID";
	...
	if ($headers) {
		...
		for($i = 1; $i <= $email_count; $i++) {
			...
			foreach ($regexp as $regexp_id => $pattern) {
				if ((preg_match("/" . $pattern . "/ims", $mail_body)) == TRUE) {$matched = TRUE;}
			}
			if ($matched == TRUE) {
				...
				# Delete just bounces?
				if ($bouncer['DeleteLevel'] == "1") {@imap_delete($conn, $i);}
			}
			# Delete all?
			if ($bouncer['DeleteLevel'] == "2") {@imap_delete($conn, $i);}
		}
		...
		foreach ($bounced_addy_array as $bouncenum => $bounced_addy) {
			...
			$query = "SELECT * FROM ".$infrespsubscribers." WHERE EmailAddress = '$bounced_addy'";
			...
			if (mysql_num_rows($result) > 0) {
				...
				$query = "DELETE FROM ".$infrespsubscribers." WHERE EmailAddress = '$bounced_addy'";
				$DB_result = mysql_query($query) or die("Invalid query: " . mysql_error());
				$query = "DELETE FROM ".infrespcustomfields." WHERE email_attached = '$bounced_addy'";
				$DB_result = mysql_query($query) or die("Invalid query: " . mysql_error());
				...
				if ($bouncer['NotifyOwner'] == "1") {
					...
					SendMessageTemplate('templates/messages/subscriber_removed.notify.txt', $bouncer['EmailAddy'],$bouncer['EmailAddy']);
				}
			}
		}
	}
...
if ($included != TRUE) {DB_disconnect();}
