...
if ($move_to_checked != TRUE) {die("Invalid responder number!<br>\n");}

# Get the other passed data
if (empty($passed['NAME'])) {$passed['NAME'] = $_REQUEST['NAME'];}
if (empty($passed['EMAIL'])) {$passed['EMAIL'] = $_REQUEST['EMAIL'];}
if (empty($passed['FIRST'])) {$passed['FIRST'] = $_REQUEST['FIRST'];}
if (empty($passed['LAST'])) {$passed['LAST'] = $_REQUEST['LAST'];}
if (empty($passed['IP'])) {$passed['IP'] = $_REQUEST['IP'];}
if (empty($passed['DEL'])) {$passed['DEL'] = $_REQUEST['DEL'];}
if ((empty($passed['DEL'])) || ($passed['DEL'] == "") || ($passed['DEL'] == 0) || ($passed['DEL'] == FALSE)) {
     $passed['DEL'] = FALSE;
}
else {$passed['DEL'] = TRUE;}
if ($passed['ONLYDEL'] == "YES") {$passed['ONLYDEL'] = TRUE;}
else {$passed['ONLYDEL'] = FALSE;
}
...
if ((empty($passed['FIRST'])) || (empty($passed['LAST']))) {
	if (!(empty($passed['NAME']))) {
		...
		if (empty($passed['LAST'])) {$passed['LAST'] = $Space_List[$Space_MaxIndex-1];}
...
foreach ($passed as $key => $value) {$safe[$key] = MakeSafe($value);}
...
if (!(isInBlacklist($safe['EMAIL']))) {
	...
	if (mysql_num_rows($result) > 0) {
		...
		if ((!(empty($safe['FIRST']))) && ($safe['FIRST'] != "")) {$user_data['FirstName'] = $safe['FIRST'];}
		if ((!(empty($safe['LAST']))) && ($safe['LAST'] != "")) {$user_data['LastName'] = $safe['LAST'];}
		if ((!(empty($safe['IP']))) && ($safe['IP'] != "")) {$user_data['IP_Addy'] = $safe['IP'];}

		# Is there any custom data?
		$query = "SELECT * FROM ".$infrespcustomfields." WHERE (email_attached= '" . $safe['EMAIL'] . "') AND resp_attached IN (". $resp_list .")";
		# echo $query . "<br>\n";
		...
		if (mysql_num_rows($custom_result) > 0) {
			...
			if ($passed['DEL'] == TRUE) {
				if ($passed['ONLYDEL'] == TRUE) {
					$query = "DELETE FROM ".$infrespcustomfields." WHERE (email_attached= '" . $safe['EMAIL'] . "') AND resp_attached IN (". $resp_list .")";
					# echo $query . "<br>\n";
				}
				else {
					$query = "DELETE FROM ".$infrespcustomfields." WHERE email_attached = '" . $safe['EMAIL'] . "' AND resp_attached = '" . $attached_responder_id . "'";
					# echo $query . "<br>\n";
				}
		...
		if ($passed['DEL'] == TRUE) {
			if ($passed['ONLYDEL'] == TRUE) {
				$query = "DELETE FROM ".$infrespsubscribers." WHERE (EmailAddress = '" . $safe['EMAIL'] . "') AND ResponderID IN (". $resp_list .")";
			}
			else {
				$query = "DELETE FROM ".$infrespsubscribers." WHERE EmailAddress = '" . $safe['EMAIL'] . "' AND ResponderID = '" . $attached_responder_id . "'";
			}
	...
	if (mysql_num_rows($result) > 0) {
		...
		if ($passed['ONLYDEL'] != TRUE) {
			...
			$query = "INSERT INTO ".$infrespubscribers." (ResponderID,SentMsgs,EmailAddress,TimeJoined,Real_TimeJoined,CanReceiveHTML,LastActivity,FirstName,LastName,IP_Addy,ReferralSource,UniqueCode,Confirmed) VALUES ($insert_values)";
			...
			if (($got_custom_fields == TRUE) && ($subscriber_id) && ($subscriber_id != NULL) && ($subscriber_id != "") && ($subscriber_id != 0)) {
				...
				$query = "INSERT INTO ".$infrespcustomfields." ($fieldstr) VALUES($valuestr)";
				# echo $query . "<br>\n";
				$custom_result = mysql_query($query) or die("Invalid query: " . mysql_error());
			}
		}
