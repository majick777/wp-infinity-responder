...
CREATE TABLE IF NOT EXISTS `InfResp_config` (
   `infinityURL` text NOT NULL default '',
   `max_send_count` bigint(32) NOT NULL default '50',
   `last_activity_trim` bigint(10) NOT NULL default '0',
   ...
   `autocall_sendmails` tinyint(1) NOT NULL default '1',
   `add_sub_size` int NOT NULL default '10',
   `subs_per_page` int NOT NULL default '100',
   `site_code` varchar(255) NOT NULL default '',
   `check_mail` tinyint(1) NOT NULL default '1',
   `check_bounces` tinyint(1) NOT NULL default '1',
   `tinyMCE` tinyint(1) NOT NULL default '4',
